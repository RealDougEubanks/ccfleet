...
async function projectExists(gitRoot, projectName) {
  ...
  // Belt-and-suspenders: confirm the resolved path is still within gitRoot.
  // isValidProjectName already blocks '/' and '..', so this never fires in
  // practice, but it gives CodeQL a statically verifiable containment proof.
  if (!path.resolve(target).startsWith(path.resolve(gitRoot) + path.sep)) return false;
  try {
