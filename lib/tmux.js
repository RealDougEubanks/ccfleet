...
async function listSessions() {
  ...
  try {
    ...
    if (msg.includes('no server running') || msg.includes('no sessions') || msg.includes('no such file or directory') || msg.includes('error connecting')) {
      return [];
    }
    throw err;
  }
...
async function createSession(gitRoot, projectName) {
  ...
  const claudeArgs = buildClaudeCommand({ remoteControlName, continueExisting });
...
async function reloadSession(sessionName, gitRoot, projectName) {
  if (!isValidSessionName(sessionName)) {
    const err = new Error('invalid session name');
    err.code = 'INVALID_SESSION';
    throw err;
  ...
  const originProjectName = await resolveProjectIdentity(cwd);
  const remoteControlName = buildRemoteControlName(originProjectName);
  // Always use --continue: if a session exists, conversation history is present.
  const claudeArgs = buildClaudeCommand({ remoteControlName, continueExisting: true });
  // respawn-pane -k kills the running process and starts a fresh one in the
  // same pane/window, preserving the tmux session and its cwd.
  await run('tmux', ['respawn-pane', '-k', '-t', `=${sessionName}`, claudeArgs.join(' ')]);
}
